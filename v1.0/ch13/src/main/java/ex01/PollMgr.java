...
public class PollMgr {
	...
	public int getMaxNum() {
		...
		try {
			...
			if (rs.next()) maxNum = rs.getInt(1);// 가장 높은 num값
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public boolean insertPoll(PollListBean plBean, PollItemBean piBean) {
		...
		try {
			con = pool.getConnection();
			sql = "insert into tblPollList(num, question,sdate,edate,wdate,type, active)"
					+ " values(talBoard_SEQ.NEXTVAL, ?,?,?,sysdate,?, ?)";
			pstmt = con.prepareStatement(sql);
			...
			if(plBean.getSdate().compareTo(plBean.getEdate()) == -1) 
				pstmt.setInt(5, 1);
			else if(plBean.getSdate().compareTo(plBean.getEdate()) == 0) 
				pstmt.setInt(5, 1);
			...
			if (result == 1) {
				...
				if (j > 0) flag = true; // 질의 실행이 정상적으로 이루어짐
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public boolean updatePoll(int num, String[] itemnum) {
		...
		try {
			con = pool.getConnection();
			sql = "update tblPollItem set cnt = cnt+1 where listnum=? and itemnum = ?";
			pstmt = con.prepareStatement(sql);
			if (num == 0) {
				num = getMaxNum();
			}
	...
	public Vector<PollItemBean> getView(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select item, cnt from tblPollItem where listnum=?";
			pstmt = con.prepareStatement(sql);
			if (num == 0)
	...
	public int sumCount(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select sum(cnt) from tblPollItem where listnum=?";
			pstmt = con.prepareStatement(sql);
