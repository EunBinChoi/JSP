...
<%	
	  ...
	  int numPerPage = 10; // 페이지당 레코드 (게시물 개수) 수 
	  ...
	  int listSize = 0; // 현재 읽어온 게시물의 수
		...
		// DB에서 start ~ end까지 게시물 가지고 옴
		 start = (nowPage * numPerPage)-numPerPage; // 10
		 end = start + numPerPage; // 10 + 10
	 
		// 전체 게시물 저장
		totalRecord = bMgr.getTotalCount(keyField, keyWord);
	
	/* 전체 페이지 개수를 계산하는 부분
	만약 122개의 레코드가 있다면 122/10의 결과값의 12.2를 올림하여 
	13개의 페이지가 만들어짐 */
	...
	/* nowPage = 1
	   ...
	   nowPage = 20
		...
		nowPage/15 ==> 몇번째 블럭인지 확인가능!
		1/15 => 0.xxxx => 1 (첫번째 블럭)
		20/15 => 1.xxxx => 2 (두번째 블럭)
	*/
	...
	<table align="center" width="600" cellpadding="3">
		...
		<tr>
			...
			<%
   				  ...
   				  int pageEnd 
   				  = ((pageStart + pagePerBlock ) <= totalPage) 
