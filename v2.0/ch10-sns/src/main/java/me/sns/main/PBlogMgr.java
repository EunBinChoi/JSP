...
import me.sns.etc.RandomMgr;
import me.sns.etc.UtilMgr;
import me.sns.heartReply.HeartReplyMgr;
import me.sns.reply.PReplyMgr;
import me.java.beans.PBlogBean;
import me.java.beans.PMemberBean;
...
public class PBlogMgr {
	...
	private static final String JSPWORKSPACE = "J:\\GoottAcademy-2\\jsp-workspace\\ch10-sns\\src\\main\\webapp";
	private static final String SAVEFOLDER = "photo";
	...
	public Vector<PMemberBean> listPMember(String id) {
		...
		ResultSet rs = null;
		String sql = null;
		Vector<PMemberBean> postMemberList = new Vector<PMemberBean>();
		try {
			con = pool.getConnection();

			// tblPMember에서 매개변수로 받은 id를 제외하고 5개의 랜덤한 회원정보 검색하는 sql문
			sql = "select * from (select id, name, profile from tblPMember "
					+ "where id !=? order by dbms_random.value) where rownum <= 5";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				...
				postMemberList.addElement(bean);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void insertPBlog(HttpServletRequest req) {
		...
		try {
			...
			sql = "insert into tblPBlog (num, message, id, pdate, photo) "
					+ "values (tblPBlog_SEQ.nextval, ?, ?, sysdate, ?)";
			pstmt = con.prepareStatement(sql);
	...
	public void deletePBlog(int num) {
		...
		try {
			...
			if (photo != null) {
				File file = new File(JSPWORKSPACE + File.separator + SAVEFOLDER + File.separator + photo);
				if (file.exists())
					UtilMgr.delete(JSPWORKSPACE + File.separator + SAVEFOLDER + File.separator + photo);
			...
			if (pstmt.executeUpdate() > 0) {
				...
				// 삭제한 포토포스트와 관련된 하트 댓글도 모두 삭제하는 메소드 호출
				hMgr.deleteAllHeartReply(num);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void downHCnt(int num) {
		...
		try {
			...
			// 조건에 맞는 포토포스트 '좋아요' (hCnt)의 카운트가 1씩 감소하는 sql문
			sql = "update tblPBlog set hCnt=hCnt-1 where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
